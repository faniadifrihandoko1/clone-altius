"use client";
import ModalDelete from "@/components/comon/custom-modal/modal-delete";
import Icon from "@/components/comon/icon";
import IconButton from "@mui/material/IconButton";
import { usePathname, useRouter } from "next/navigation";
import { useState } from "react";
import { SelectMember } from "../modal/select-member";

interface ListOptionsPropsType {
  data: any;
}

export const ListOptions = (props: ListOptionsPropsType) => {
  const { data } = props;
  const route = useRouter();
  const pathname = usePathname();

  const [openSelectMember, setOpenSelectMember] = useState<boolean>(false);
  const [openDelete, setOpenDelete] = useState<boolean>(false);

  const toggleSelectMember = () => setOpenSelectMember(!openSelectMember);
  const toggleDelete = () => setOpenDelete(!openDelete);

  const handleEdit = () => {
    route.push(`${pathname}/edit/` + data.id);
  };
  return (
    <>
      <IconButton size="small" title="Edit" onClick={handleEdit}>
        <Icon icon="system-uicons:create" />
      </IconButton>
      <IconButton
        size="small"
        title="Pilih Anggota Grup Akses Scorecard Organisasi"
        onClick={toggleSelectMember}
      >
        <Icon icon="ant-design:select-outlined" />
      </IconButton>
      <IconButton size="small" title="Delete" onClick={toggleDelete}>
        <Icon icon="mingcute:delete-2-line" />
      </IconButton>

      {openSelectMember && (
        <SelectMember
          openDialog={openSelectMember}
          toggleDialog={toggleSelectMember}
        />
      )}
      {openDelete && (
        <ModalDelete
          name={data.name}
          open={openDelete}
          toggle={toggleDelete}
          handleDelete={toggleDelete}
        />
      )}
    </>
  );
};
